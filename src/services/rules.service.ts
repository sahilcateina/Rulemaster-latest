...
//   rulesDAO.updateRule(id, updates);

// export const deleteRule = async (id: string) => rulesDAO.deleteRule(id);

// export const getAllRules = async (tenantId?: string, groupId?: string) =>
//   rulesDAO.getAllRules(tenantId, groupId);

// export const applyRules = async (tenantId: string, inputData: any, groupId: string) => {
//   const { data: rules, error } = await rulesDAO.getAllRules(tenantId, groupId);
//   if (error) throw new Error('Failed to fetch rules');
