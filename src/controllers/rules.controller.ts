...
//       id: uuidv4(),
//       tenant_id: req.body.tenant_id,
//       group_id: req.body.group_id, // Include group_id
//       name: req.body.name,
//       description: req.body.description,
//       conditions: req.body.conditions,
//       actions: req.body.actions,
...
//     const { data, error } = await service.getAllRules(tenant_id, group_id);
//     if (error) throw error;
//     res.json({ status: 200, data });
//   } catch (err: any) {
//     res.status(500).json({ error: err.message });
...
//   try {
//     const prompt: string = req.body.prompt;
//     const tenant_id: string = req.body.tenant_id || req.headers['tenant-id'] as string;
//     const group_id: string = req.body.group_id;

//     if (!prompt || !tenant_id || !group_id) {
//       res.status(400).json({ error: 'Prompt, tenant_id and group_id are required' });
//       return;
//     }

//     const saved = await service.parseAndSave(prompt, tenant_id, group_id);
//     res.status(200).json({ status: 200, data: saved.rule });
//   } catch (err: any) {
//     console.error('Rule creation failed:', err.message);
//     res.status(500).json({ error: err.message });
...
// export const applyRules = async (req: Request, res: Response): Promise<void> => {
//   try {
//     const { tenant_id, group_id, input } = req.body;

//     if (!tenant_id || !group_id || !input) {
//       res.status(400).json({ error: "tenant_id, group_id, and input are required" });
//       return;
//     }

//     const events = await service.applyRules(tenant_id, input, group_id);
//     res.status(200).json({ status: 200, events });
//   } catch (err: any) {
//     res.status(500).json({ error: err.message });
//   }
